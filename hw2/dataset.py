...
class TextDataset(Dataset):
    ...
    def __init__(self, data_file: str, train: bool = True, sp_model_prefix: str = None,
        ...
        if not os.path.isfile(sp_model_prefix + '.model'):
            ...
            SentencePieceTrainer.train(
                ...
                normalization_rule_name=normalization_rule_name, pad_id=7,
            )
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, int]:
        ...
        indices = torch.full((self.max_length, ), self.pad_id)
        length = len(self.indices[item]) + 2
        if length <= self.max_length:
            indices[:length] = torch.tensor([self.bos_id] + self.indices[item] + [self.eos_id])
        else:
            indices[:self.max_length] = torch.tensor([self.bos_id] + self.indices[item][:self.max_length-2] + [self.eos_id])
